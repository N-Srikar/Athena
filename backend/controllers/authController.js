  ...
  try {
    ...
    if (email === config.ADMIN_EMAIL) {
      if (password === config.ADMIN_PASSWORD) {
        ...
        return res.status(200).json({ message: 'Admin logged in', token ,role: 'admin' , name:'Srikar'});
      } else {
    ...
    res.status(200).json({ message: 'Logged in successfully', token , role:user.role , name:user.name});
  } catch (error) {
